...
def main(market=None):
    ...
    ---
    寫作規則:
    撰寫一份**目標長度約為3000個繁體中文字，不要超過4000個繁體中文字**的專業分析報告。
    ...
    請基於這些資訊，為我提供一份聚焦當日的{market_name}市場動態摘要報告。
    報告段落如下，不要新增或減少段落：
    1.  **焦點產業**：哪些產業或概念股是這段時間內的{market_name}市場焦點？為什麼受到關注？
    2.  **關鍵公司動態**：提及至少三家在這批新聞中最重要的{market_name}公司，並說明它們發生了什麼關鍵事件（如財報、法說會、重大消息等）。
    3.  **市場重點分析**：總結這段時間內{market_name}市場的整體氣氛、主要指數表現，以及當日最值得關注的重大事項，最後以兩到三句話收尾歸納今日核心主軸。
